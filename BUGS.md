# Created: 2026-02-20
# Last updated: 2026-02-26 — Add OOGGA scoring discussion to BUG-006, add Potapov Table 1 overhang data
...
- **Potapov Table 1 — 25-overhang set (to be embedded in code):**
  - Source: Potapov et al. 2018, ACS Synth Bio, Table 1. Optimized via simulated annealing for **set fidelity** (product of pairwise correct fractions). Predicted 95.8% set fidelity — highest for any 25-overhang set in the paper.
  - Exact sequences: TBD (deep research in progress to extract from paper/supplementary)
  - This replaces the current greedy-generated `potapov_set2_20` set.
- **OOGGA scoring discussion — two candidate approaches:**
  - OOGGA (Overhang Optimizer for Golden Gate Assembly, bioRxiv 2025, DOI 10.1101/2025.06.16.659877, GitHub: bigbigdumdum/OOGGA) uses a DP that scores overhangs by pairwise ligation fidelity from the 256×256 Potapov matrix.
  - **Option A — Pure OOGGA DP:** Score overhangs purely by pairwise matrix entries. The DP naturally finds high-fidelity combinations without reference to a pre-computed HF set. Pros: simpler, no dependency on pre-computed sets. Cons: might miss higher-order interactions that the Potapov simulated annealing captured.
  - **Option B — OOGGA DP + HF set upscoring:** Same pairwise scoring, but add a bonus term when an overhang is in the Potapov Table 1 HF set. This biases the DP toward experimentally validated overhangs. Pros: leverages both pairwise data and set-level optimization. Cons: slightly more complex, bonus weight needs tuning.
  - **Key question:** Does the HF set bonus add value beyond what pure pairwise scoring captures? If the Potapov sets were optimized via simulated annealing that considered higher-order interactions (not just pairwise), then the bonus encodes information not in the pairwise matrix. If the SA only used pairwise data, the bonus is redundant.
  - **Decision:** Pending deep research on OOGGA scoring details. Will be resolved before implementing the scoring overhaul.
  - Investigation in progress (branch `260225-DP-OOGGA`).
